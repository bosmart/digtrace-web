...
def main():
		...
		# Read method from client and call the relevant method
		...
		if method:
			if method == "rotate90":
				return rotate90( data, precision )
			elif method == "autoRotateCreatePanel":
				return autoRotateCreatePanel( data )
			elif method == "interpolate":
				return interpolate( data, precision, "linear", None, None, False, multiplier )
			elif method == "interpolate3D":
				return interpolate3D( data )
			elif method == "autoRotate":
				return autoRotate( data, precision )
			elif method == "mirror":
				return mirror( data )
			elif method == "depthChart":
				return depthChart( data, landmarks )
			elif method == "contour":
				return contour( data, xyzi, landmarks1, landmarks2, line, triangle, circle, transformation, contourLines )
			elif method == "statistics":
				return statistics( models, precision, scale )
			else:
...
def interpolate( data, precision, method="linear", mn=None, mx=None, internal=False, multiplier=1 ):
	...
	if internal == False:
		...
		return jsonify( json.dumps( jsonArray ) )
...
def contour	( xyzi1, xyzi2, landmarks1, landmarks2, line, triangle, circle, transformation, contourLines ):
	...
	if np.sum(nan_mask) > 2 and landmarks2.shape[0] == landmarks1.shape[0]:
		...
		# transform landmark-panel coordinates (0..X, 0..Y) to interpolated coordinates
		xm = xyzi1[3]
		ym = xyzi1[4]
		z = xyzi1[2]
...
def normalize_z_axis( xyz, xyzi, val, json=False ):
