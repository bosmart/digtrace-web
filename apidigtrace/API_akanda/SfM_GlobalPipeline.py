...
# OPENMVG_SFM_BIN =  os.path.join(my_path, "../lib/Unix/")
...
class Global_pipeline:
    ...
    def pipeline(self):
        ...
        pMatches = subprocess.Popen([os.path.join(self.OPENMVG_SFM_BIN, "openMVG_main_ComputeMatches"), "-i",
                                     self.matches_dir + "/sfm_data.json", "-o", self.matches_dir, "-g", "e"])
        pMatches.wait()
        ...
        pRecons = subprocess.Popen(
            [os.path.join(self.OPENMVG_SFM_BIN, "openMVG_main_GlobalSfM"), "-i", self.matches_dir + "/sfm_data.json",
             "-m", self.matches_dir, "-o", self.reconstruction_dir])
        pRecons.wait()

        # print ("5. Colorize Structure")
        # pRecons = subprocess.Popen( [os.path.join(self.OPENMVG_SFM_BIN, "openMVG_main_ComputeSfM_DataColor"),  "-i", self.reconstruction_dir+"/sfm_data.bin", "-o", os.path.join(self.reconstruction_dir,"colorized.ply")] )
        # pRecons.wait()
        #
        # # optional, compute final valid structure from the known camera poses
        # print ("6. Structure from Known Poses (robust triangulation)")
        # pRecons = subprocess.Popen( [os.path.join(self.OPENMVG_SFM_BIN, "openMVG_main_ComputeStructureFromKnownPoses"),  "-i", self.reconstruction_dir+"/sfm_data.bin", "-m", self.matches_dir, "-f", os.path.join(self.matches_dir, "matches.e.bin"), "-o", os.path.join(self.reconstruction_dir,"robust.bin")] )
        # pRecons.wait()
        #
        # pRecons = subprocess.Popen( [os.path.join(self.OPENMVG_SFM_BIN, "openMVG_main_ComputeSfM_DataColor"),  "-i", self.reconstruction_dir+"/robust.bin", "-o", os.path.join(self.reconstruction_dir,"robust_colorized.ply")] )
        # pRecons.wait()
